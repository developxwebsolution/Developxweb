...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <JsonLd
        data={[
          articleSchema({ title: post.title, description: post.excerpt, path: `/blog/${post.slug}`, date: post.date, author: post.author }),
          breadcrumbSchema([
            { name: "Home", path: "/" },
            { name: "Blog", path: "/blog" },
      ...
      <section className="border-b border-line bg-paper-raised py-16 sm:py-20">
        <Container className="max-w-2xl">
          ...
          <div className="mt-6 flex flex-col gap-4">
            {more.map((p) => (
              <Link key={p.slug} href={`/blog/${p.slug}`} className="rounded-xl border border-line p-5 hover:border-indigo">
                <span className="font-mono text-xs uppercase tracking-wider text-indigo">{p.category}</span>
